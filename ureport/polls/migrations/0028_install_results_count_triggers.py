...
BEGIN
  ...
  IF _poll_result.org_id IS NOT NULL AND _poll_result.flow IS NOT NULL THEN
    ...
    -- The number of completed runs; contacts that went into the entire flow
    IF _poll_result.completed IS TRUE THEN
      PERFORM ureport_insert_results_counter(_poll_result.org_id, _poll_result.ruleset, CONCAT('completed:', LOWER(_poll_result.flow)), _count);
    END IF;

    IF _poll_result.ruleset IS NOT NULL THEN
      -- We have a ruleset, count the number that reached the ruleset; number of contacts the received this question
